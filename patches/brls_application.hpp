...
class Application
{
  public:
    ...
    // Press-and-hold-OK shim. When enabled, the BUTTON_A click action
    // is *deferred* — instead of firing on press, it fires on release
    // ONLY if the press was shorter than holdAToOpenMenuThresholdMs.
    // If the user holds longer than the threshold, BUTTON_A is treated
    // as a synthesized BUTTON_GUIDE press (which the app re-dispatches
    // to BUTTON_START in main_activity.cpp, opening the context menu).
    // Default is off so every other platform keeps the original
    // press-time click semantics; main_activity.cpp turns it on for
    // Android TV at app startup.
    inline static void setHoldAToOpenMenu(bool enable, int thresholdMs = 600)
    {
        holdAToOpenMenu = enable;
        holdAToOpenMenuThresholdMs = thresholdMs;
    }
    inline static bool isHoldAToOpenMenuEnabled() { return holdAToOpenMenu; }
  ...
  private:
    ...
    // See setHoldAToOpenMenu above. holdAStartedAt is the cpuTime
    // (microseconds) at which the current BUTTON_A press began, or 0
    // when not pressed. holdAFiredAsMenu is set true on the frame we
    // synthesize the GUIDE press so we don't re-fire it for every
    // subsequent frame the button is still down.
    inline static bool holdAToOpenMenu = false;
    inline static int  holdAToOpenMenuThresholdMs = 600;
    inline static Time holdAStartedAt = 0;
    inline static bool holdAFiredAsMenu = false;
