...
{
    // VitaPlex patch: the upstream loop trapped itself in a blocking
    // glfwWaitEvents() while the window was iconified (a do/while that only
    // exits once the window is restored). That also froze brls::Threading sync
    // task processing in internalMainLoop(), so OS media-control transport
    // posted from the MPRIS D-Bus thread (play/pause/next/...) never ran while
    // the window was minimized — the desktop media widget buttons did nothing.
    //
    // Wait on a timeout instead and return every iteration, so background timers
    // and sync tasks keep ticking (at the deactivated frame rate) while
    // minimized; the media widget then controls playback normally. The timeout
    // throttles the loop, so this doesn't busy-spin.
    bool isActive = !glfwGetWindowAttrib(this->videoContext->getGLFWWindow(), GLFW_ICONIFIED);
