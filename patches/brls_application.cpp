...
void Application::processInput()
{
    ...
    {
        // Hold-A-to-open-menu shim. When enabled (e.g. on Android TV
        // via Application::setHoldAToOpenMenu), defer BUTTON_A click
        // firing to button release and turn long-holds into a
        // synthetic BUTTON_GUIDE press. Every other button behaves
        // exactly as before, and when the shim is off this branch is
        // a single bool check so non-Android paths take the original
        // code unmodified.
        if (i == BUTTON_A && holdAToOpenMenu)
        {
            if (controllerState.buttons[BUTTON_A])
            {
                if (!oldControllerState.buttons[BUTTON_A])
                {
                    // Just pressed — start the timer, don't fire yet.
                    holdAStartedAt   = cpuTime;
                    holdAFiredAsMenu = false;
                }
                else if (!holdAFiredAsMenu && holdAStartedAt > 0)
                {
                    Time elapsedUs = cpuTime - holdAStartedAt;
                    if (elapsedUs >= static_cast<Time>(holdAToOpenMenuThresholdMs) * 1000)
                    {
                        // Held past the threshold — synthesize a GUIDE
                        // press, which the app routes to BUTTON_START
                        // (the Options/Menu action) via the rootBox
                        // handler in main_activity.cpp.
                        holdAFiredAsMenu = true;
                        Application::onControllerButtonPressed(BUTTON_GUIDE, false);
                    }
                }
            }
            else if (oldControllerState.buttons[BUTTON_A])
            {
                // Just released. If we never crossed the hold
                // threshold, this was a tap — fire the deferred click
                // now. Otherwise the GUIDE press already fired; the
                // click is intentionally swallowed.
                if (!holdAFiredAsMenu)
                    Application::onControllerButtonPressed(BUTTON_A, false);
                holdAStartedAt   = 0;
                holdAFiredAsMenu = false;
            }
            controllerState.repeatingButtonStop[BUTTON_A] = 0;
            continue;
        }
